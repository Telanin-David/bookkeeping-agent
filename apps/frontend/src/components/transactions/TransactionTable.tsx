...
const statusColor: Record<Transaction['status'], 'green' | 'yellow' | 'red' | 'gray'> = {
  completed: 'green',
  pending:   'yellow',
  overdue:   'red',
  cancelled: 'gray',
...
const typeColor: Record<Transaction['type'], 'green' | 'red' | 'blue' | 'yellow' | 'gray'> = {
  income:      'green',
  expense:     'red',
  receivable:  'blue',
  payable:     'yellow',
  ...
  return (
    <Table
      columns={[
        ...
        { key: 'type',        header: 'Type',         render: (r) => <Badge color={typeColor[r.type]}>{capitalize(r.type)}</Badge> },
        ...
        { key: 'status',      header: 'Status',       render: (r) => <Badge color={statusColor[r.status]}>{capitalize(r.status)}</Badge> },
      ]}
